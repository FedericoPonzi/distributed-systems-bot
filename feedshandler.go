...
func (handler FeedHandler) main() {
	...
	if len(feedItemsToPublish) > 0 {
		...
		handler.twitterHandler.publishLinkWithTitle("This is a test. Sorry for the inconvenience." + feedItemsToPublish[0].Title, feedItemsToPublish[0].Link)
...
func (handler FeedHandler) getUpdatedFeedsItemWorker(jobs <-chan FeedUpdatedWorkUnit, results chan<- gofeed.Item, wg *sync.WaitGroup) {
	for j := range jobs {
		...
		if lastUpdated.Before(*feed.updated) {
			...
			log.Println("It was updated after yersterday: ", lastUpdated, " is after: ", feed.updated)
		}
	}
	wg.Done()
}
